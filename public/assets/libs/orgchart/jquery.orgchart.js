(function ($) {
    ...
    function OrgChart($container, opts) {
        ...
        this.startEdit = function (id) {
            ...
            $('input[name="parent_id"]').val(nodes[id].data.parent_id);
            ...
            $('.submit-update-category').on('click', function () {
                ...
                nodes[id].data.parent_id = form.find('input[name="parent_id"]').val();
                ...
                $(`.node[node-id=${id}] span`).text(nodes[id].data.description);
            })
        }
        ...
        this.deleteNode = function (id) {
            ...
            nodes[nodes[id].data.parent_id || 0].removeChild(id);
            delete nodes[id];
            self.draw();
        }
        ...
        for (var i in nodes) {
            if (nodes[i].data.parent_id === -1) {
                ...
                nodes[nodes[i].data.parent_id || 0].addChild(nodes[i]);
            }
        }
    ...
    function Node(data) {
        ...
        this.formatNode = function (opts) {
            ...
            if (typeof data.description !== 'undefined') {
                descString = '<span>' + self.data.description + '</span>';
            }
            if (opts.showControls) {
                var buttonsHtml = "<div class='org-add-button'>" + opts.newNodeText + "</div>";
                if (this.data.id !== 0) {
                    buttonsHtml += "<div class='org-del-button'><i class='dripicons-minus'></i></div>";
                }
            } else {
                buttonsHtml = '';
            }
